...
function getDetailIntro() {
    $.ajax({
        type: "GET",
        url: `http://localhost:8080/near/${getId()}`,
        data: {},
        async: false,
...
function weather() {
    ...
    $.ajax({
        type: "POST",
        url: "http://localhost:8080/near/weather",
...
function toggle_bookmark(content_id) {
    ...
    if (!localStorage.getItem('token')) {
        ...
        if ($('#bookmark').hasClass("fas")) {
            $.ajax({
                type: "POST",
                url: "http://localhost:8080/near/bookmark",
                contentType: "application/json",
                data: JSON.stringify({
...
function getBookmark() {
    $.ajax({
        type: "GET",
        url: `http://localhost:8080/near/bookmark/${getId()}`,
        data: {},
        success: function (response) {
